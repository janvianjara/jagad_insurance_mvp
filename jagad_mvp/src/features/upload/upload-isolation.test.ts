...
/**
 * §11.1, enforced rather than promised — the upload page's half.
 *
 ...
 * the walk below starts at the upload screen, follows every RUNTIME import
 ...
 * upload screen. The only route to it is `lazy(() => import(...))` in
...
const ENTRY = '/src/features/upload/UploadTokenScreen.tsx'
...
const FORBIDDEN: readonly string[] = [
  ...
  '/src/features/kyc',
  '/src/features/claims',
  '/src/features/consent/ConsentTokenScreen.tsx',
...
describe('the upload page is a chunk of its own, and it carries no session', () => {
  ...
  it('reaches nothing that assumes a signed-in user', () => {
    ...
    expect(modules).toContain('/src/features/upload/data/upload-desk.ts')

    const breaches = modules.filter((module) =>
      FORBIDDEN.some((forbidden) => module === forbidden || module.startsWith(`${forbidden}/`)),
    )
    expect(breaches, 'the upload page reached the authenticated side of the app').toEqual([])
  })
  ...
  it('is pulled in by a dynamic import and nothing else', () => {
    expect(SOURCES['/src/features/upload/routes.ts']).toMatch(
      /lazy\(\(\)\s*=>\s*import\('\.\/UploadTokenScreen'\)\)/,
    )
